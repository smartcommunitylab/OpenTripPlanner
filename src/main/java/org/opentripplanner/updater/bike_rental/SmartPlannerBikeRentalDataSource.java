...
public class SmartPlannerBikeRentalDataSource implements BikeRentalDataSource, JsonConfigurable {

	private static final Logger log = LoggerFactory.getLogger(SmartPlannerBikeRentalDataSource.class);

	private String url;

	ArrayList<BikeRentalStation> stations = new ArrayList<BikeRentalStation>();

	public SmartPlannerBikeRentalDataSource() {
	...
	private void parseJson(String data) throws ParserConfigurationException, SAXException, IOException {
		...
		for (JsonNode stationNode : mapper.readTree(data)) {
			...
			brstation.y = stationNode.path("position").get(0).asDouble();
			brstation.x = stationNode.path("position").get(1).asDouble();
			brstation.name = stationNode.path("name").asText();
			brstation.bikesAvailable = stationNode.path("bikes").asInt();
			brstation.spacesAvailable = stationNode.path("slots").asInt();
			brstation.address = stationNode.path("address").toString();
